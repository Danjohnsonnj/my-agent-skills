...
cold-start-resumable handoff brief tree (`AGENTS.md` pointer + `docs/_handoff/*`) for
...
session". The skill scaffolds the brief tree from `templates/`, wires up the `AGENTS.md`
pointer, and maintains the artifacts on each wrap-up. See `SKILL.md` for the workflow
and `reference.md` for adoption modes, `AGENTS.md` merge logic, and a worked example.

## Layout

- `SKILL.md` - slim skill body (entry point).
- `reference.md` - methodology detail (adoption modes, merge logic, worked example).
- `templates/` - parameterized artifact templates (`AGENTS.snippet.md`, `HANDOFF.md`,
