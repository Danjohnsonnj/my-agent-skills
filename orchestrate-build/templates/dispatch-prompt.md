<!-- The literal cold-start prompt the orchestrator launches a subagent with,
     after overwriting {{PLAN_ROOT}}/_bus/handoff.md for this chunk. Pass it
     ...
     Substitute {{PLAN_ROOT}} with the active plan folder
     (e.g. docs/plans/skills-explainer-site). -->

Cold-start sub-session. Do not assume any prior context.

1. Read `{{PLAN_ROOT}}/HANDOFF.md` -> the "Required reading" listed
   in `{{PLAN_ROOT}}/_bus/handoff.md`.
2. Do exactly the chunk described in `{{PLAN_ROOT}}/_bus/handoff.md`. Stay in
...
4. STOP and hand back early on any of: blocked, ambiguous, 2nd failure on the
   same step, scope-creep. Report any of these via the `blocked` status.
5. Do NOT commit. Leave the working tree green if your status is `complete`.
6. Final action: overwrite `{{PLAN_ROOT}}/_bus/handback.md` from
