...
| Link form | Paths the executor must load → markdown links (machine-checked by `check-refs.py`). Exception: under HANDOFF **Required reading (this phase)**, plain `docs/_handoff/*.md` paths are OK (plan-build convention). Elsewhere, use markdown links. |
...
```markdown
## Required reading
- [tech-brief](docs/_handoff/tech-brief.md) — why load it
...
Run only when the plan references `docs/_handoff/` or `HANDOFF.md`:
